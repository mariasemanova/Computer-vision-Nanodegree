...
class Net(nn.Module):
    ...
    def __init__(self):
        ...
        #self.fc1 = nn.Linear(16224, 10000)
        #self.fc2 = nn.Linear(10000, 1500)
        self.fc3 = nn.Linear(64*4*4, 136)
    ...
    def forward(self, x):
        ...
        x = self.fc_drop1(x)
        x = F.dropout(self.pool2(F.relu(self.conv2(x))))
        x = self.fc_drop2(x)
        x = F.dropout(self.pool3(F.relu(self.conv3(x))))
        x = self.fc_drop3(x)
        x = F.dropout(self.pool4(F.relu(self.conv4(x))))
        x = self.fc_drop4(x)
